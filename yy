...
local aa = {
	...
	[26] = function()
		...
		function c.New(d, e, f)
			...
			function h.SetValueFromText(value)
				local numericValue = tonumber(value)
				if numericValue then
					h:SetValue(math.clamp(numericValue, h.Min, h.Max))
				else
					print("Invalid input")
				end
			end

			-- ฟังก์ชันตั้งค่าค่าของ Slider
			function h.SetValue(p, s)
				p.Value = g:Round(math.clamp(s, h.Min, h.Max), h.Rounding)
				n.Text = tostring(p.Value) -- อัปเดตค่าใน TextBox
				g:SafeCallback(h.Callback, p.Value)
			end

			-- ผูกการเปลี่ยนค่า TextBox กับ Slider
			ah.AddSignal(
				n.FocusLost,
				function(enterPressed)
					if enterPressed then
						h:SetValueFromText(n.Text)  -- เมื่อ Focus Lost ให้เรียก SetValueFromText
					end
				end
			)

			h:SetValue(f.Default)  -- ตั้งค่าค่าตั้งต้น
			g.Options[e] = h
			return h
		end
	...
	[47] = function()
		...
		local af = {Names = {"Dark", "Darker", "Light", "Aqua", "Amethyst", "Crimson"}}
	...
	[50] = function()
		...
		return {
			Name = "Dark",
			Accent = Color3.fromRGB(255, 0,73, 255),
